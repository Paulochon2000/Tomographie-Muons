...
int main(){
	...
	TH1F prod1("prod1","Sans objet",nbins,0,2*pi);
	TH1F prod2("prod2","Avec objet",nbins,0,2*pi);
	...
	while (!recdoc.eof()){
		...
		prod2.Fill(phi_rec);// remplissage de l'histogramme.
	...
	while (!sansdoc.eof()){
		...
		prod1.Fill(phi);//remplissage de l'histogramme.
	...
	prod1.GetYaxis()->SetTitle("nombre d'apparition");

	
	graph.Print("phi_objet.png"); //generer et enregistrer sous ce nom.
}
