...
int main(){
	...
	// Nous prenons les caracteristiques du systeme. Les notations sont definies dans le doxument donne.txt .
	...
	// chargeons les particules arrivant au plan ( on ignore l'objet ici).
	...
	ofstream x_doc("x_sans_obj.txt");
	ofstream y_doc("y_sans_obj.txt");
	ofstream angles_doc("angles_simule_sans_obj.txt");
	
	
	
	int N_2=0; //nombre de particules au final.
	...
	while (!particules.eof())
	{
		...
		if ( abs(muon.Getx())< demi_l && abs(muon.Gety())<demi_L)
			...
			//recuperons les coordonnes pour chaque plan.
			for (int i=1; i<N;i++){
