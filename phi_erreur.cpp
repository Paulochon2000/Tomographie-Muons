...
int main(){
	...
	double pi = 4*atan(1);
	TH1F prod("prod","erreur sur l'angle phi",nbins,0,1); // creer un histogramme.
	...
	while (!recdoc.eof()){
		...
		prod.Fill(error_phi); // remplissage de l'histogramme.
	...
	// nom des axes.
	prod.GetXaxis()->SetTitle("erreur sur phi");
	...
	graph.Print("phi_erreur.png"); // afficher et enregistrer l'image.
}
