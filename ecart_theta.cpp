...
int main(){
	...
	double pi = 4*atan(1);
	TH1F prod("prod","ecart relatif entre theta simule et reconstruit",nbins,-1,1);// creer un histogramme.
	...
	while (!simudoc.eof()){
		...
		ecart = (theta_rec -theta)/theta;// definition de l'ecart relatif.
	...
	// nom des axes.
	prod.GetXaxis()->SetTitle("ecart relatif sur theta");
	...
	graph.Print("theta_ecart.png"); // afficher et enregistrer l'image.
}
